...
st.markdown(
    ...
    div[data-testid="stTextInput"][data-key="z_niva"],
    div[data-testid="stTextInput"][data-key="z_Q2"] {
        max-width: 150px;
    }
    </style>
    """,
...
with col_out:
    ...
    if fundament_i_vatten and z_v and z_v > 0:
        ...
        ax.hlines(
            y=z_v,
            xmin=-max_d-1, xmax=max_d+1,
            colors="blue", linestyles="--", linewidth=2
        )
        # måttsättning z_v
        ax.annotate(
            "", xy=(max_d+0.5, 0), xytext=(max_d+0.5, z_v),
            arrowprops=dict(arrowstyle="<->", color="blue")
        )
        ax.text(
            max_d+0.7, z_v/2, r"$z_{v}$",
    ...
    if Qk_H1>0:
        ...
        ax.text(
            -D_s/2-pil_längd_extra/2 - zQ1_x_offset,
            z_Q1+0.3,
            r"$Q_{k,H1}$", color="red",
        ...
        ax.text(
            -D_s/2-pil_längd_extra-0.1-zQ1_x_offset,
            z_Q1/2,
            r"$z_{Q1}$", va="center",
    ...
    if Qk_H2>0:
        ...
        ax.text(
            -D_s/2-pil_längd_extra/2 - zQ2_x_offset,
            z_Q2+0.3,
            r"$Q_{k,H2}$", color="red",
            ha="center", fontsize=14
        )
        ...
        ax.text(
            -D_s/2-pil_längd_extra-0.1-zQ2_x_offset,
            z_Q2/2,
            r"$z_{Q2}$", va="center",
            color="red", fontsize=12
        )
    ...
    if Gk_ovr>0:
        ...
        ax.text(
            0, pil_längd_extra_vert+0.3,
            r"$G_{k,\mathrm{övrigt}}$", color="red",
            ha="center", fontsize=14
        )
...
with col_res:
    ...
    if fundament_i_vatten and z_v>0:
        under_b = max(0, min(z_v, H_b))*pi*(D_b/2)**2
        over_b  = vol_b - under_b
        under_s = max(0, min(z_v-H_b, H_s))*pi*(D_s/2)**2
        over_s  = vol_s - under_s
    else:
        under_b, over_b = 0, vol_b
        under_s, over_s = 0, vol_s

    # Egenvikter
    vikt_ov = (over_b+over_s)*25
    vikt_un = (under_b+under_s)*15
    vikt_tot= vikt_ov+vikt_un
